...
def create_dataset() -> [array, list]:
    group = array([1.0, 1.1], [1.0, 1.0], [0, 0], [0, 0.1])
    labels = ['A', 'A', 'B', 'B']
    return group, labels


def classify0(inx: array, data_set: array, labels: list, k: int) -> int:
    ...
    sorted_class_cnt = sorted(class_cnt.iteritems(),
                              key=operator.itemgetter(1), reversed=True)
    return sorted_class_cnt[0][0]


def file2matrix(file_name: str) -> [array, list]:
...
def data_show(data_set: array, labels:list):
    fig = plt.figure()
...
def auto_norm(data_set: array) -> [array, int, int]:
...
def dating_class_test():
    ...
    for i in ranges(num_test_vecs):
        class_res = classify0(norm_mat[i, :], norm_mat[num_test_vecs:m, :],
